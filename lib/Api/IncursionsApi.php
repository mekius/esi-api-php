...
class IncursionsApi
{
     ...
     * @return \ESI\Model\GetIncursions200Ok[]
     */
     ...
     * @param string $datasource The server name you would like data from (optional, default to tranquility)
     * @throws \ESI\ApiException on non-2xx response
     * @return array of \ESI\Model\GetIncursions200Ok[], HTTP status code, HTTP response headers (array of strings)
     */
    public function getIncursionsWithHttpInfo($datasource = null)
    {
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\ESI\Model\GetIncursions200Ok[]',
                '/incursions/'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\ESI\Model\GetIncursions200Ok[]', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\ESI\Model\GetIncursions200Ok[]', $e->getResponseHeaders());
                    $e->setResponseObject($data);
                    break;
                case 500:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\ESI\Model\GetIncursionsInternalServerError', $e->getResponseHeaders());
                    $e->setResponseObject($data);
                    break;
            }
